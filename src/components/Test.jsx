...
const Canvas = ({ selectedFrame }) => {
  ...
  const addStencil = () => {
    ...
    let prevAngle = stencilBorder.angle;
    stencilBorder.on('rotating', () => {
      const angle = stencilBorder.angle;
      const delta = angle - prevAngle;
      // rotate image
      if (imageRef.current) {
        imageRef.current.rotate((imageRef.current.angle || 0) + delta);
        imageRef.current.setCoords();
      }
      // rotate clip
      clipPath.rotate((clipPath.angle || 0) + delta);
      clipPath.setCoords();
      prevAngle = angle;
